function surfacedata = torussurface_J
%R = 4;
%r = 1;
R = 2;
r = 1;
...
R = 2;
r = 1;
fd=@(p) (sqrt(sum(p.^2,2)+R^2-2*R*sqrt(p(:,1).^2+p(:,2).^2))-r);
%R^2 -2R*sqrt(x^2 + y^2) + x^2 + y^2 + z^2 = r^2 
%Using R = 4 and r = 1.
%Starting with edge length h = .8
[node,elem]=distmeshsurface(fd,@huniform,.5,[-3.5,-3.5,-1.5;3.5,3.5,1.5]);
